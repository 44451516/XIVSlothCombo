...
namespace XIVSlothComboX.Combos.PvE
{
    internal static class DRG
    {
        ...
        {
            public const ushort TrueNorth = 1250,
                LanceCharge = 1864,
                RightEye = 1910,
                BattleLitany = 786,
        ...
        {
            public const string DRG_ST_DiveOptions = "DRG_ST_DiveOptions",
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                if (actionID is TrueThrust)
                {
                    ...
                    if (InCombat())
                    {
                        ...
                        if (inOpener)
                        {
                            ...
                            if (CanWeave(actionID))
                            {
                                ...
                                if (WasLastWeaponskill(FangAndClaw))
                                {
                                    ...
                                    {
                                        if (GetRemainingCharges(SpineshatterDive) < 2
                                            && !WasLastAction(SpineshatterDive))
                                ...
                                if (WasLastWeaponskill(HeavensThrust)
                                    && GetRemainingCharges(SpineshatterDive) > 0
                                    && !WasLastAction(SpineshatterDive)
                                    && openerOptions is 0 or 1)
                                    return SpineshatterDive;
                            }
                        }
                            ...
                            if (CanWeave(actionID))
                            {
                                ...
                                if (HasEffect(Buffs.PowerSurge))
                                {
                                    ...
                                    if (IsEnabled(CustomComboPreset.DRG_ST_CDs)
                                        && IsEnabled(CustomComboPreset.DRG_ST_Wyrmwind)
                                    ...
                                    if (IsEnabled(CustomComboPreset.DRG_ST_CDs))
                                    {
                                        ...
                                        if (IsEnabled(CustomComboPreset.DRG_ST_GeirskogulNastrond)
                                            && LevelChecked(Geirskogul)
                                            && ((gauge.IsLOTDActive && IsOffCooldown(Nastrond))
                                        ...
                                        if (IsEnabled(CustomComboPreset.DRG_ST_LifeSurge)
                                            ...
                                            && (((HasEffect(Buffs.RightEye) || HasEffect(Buffs.LanceCharge))
                                                 && lastComboMove is VorpalThrust)
                                                || (HasEffect(Buffs.BattleLitany)
                                                    && ((HasEffect(Buffs.EnhancedWheelingThrust)
                                                         && WasLastWeaponskill(FangAndClaw))
                                                        || HasEffect(Buffs.SharperFangAndClaw)
                                                        && WasLastWeaponskill(WheelingThrust)))))
                                            return LifeSurge;

                                        //Dives Feature
                                        if (IsEnabled(CustomComboPreset.DRG_ST_Dives)
                                            && (IsNotEnabled(CustomComboPreset.DRG_ST_Dives_Melee)
                                                || (IsEnabled(CustomComboPreset.DRG_ST_Dives_Melee)
                                                    && GetTargetDistance() <= 1)))
                                        {
                                            if (diveOptions is 0 or 1 or 2 or 3
                                                && gauge.IsLOTDActive
                                                && ActionReady(Stardiver)
                                                && IsOnCooldown(DragonfireDive))
                                                return Stardiver;

                                            if (diveOptions is 0 or 1
                                                || //Dives on cooldown
                                                (diveOptions is 2
                                                 && ((gauge.IsLOTDActive && LevelChecked(Nastrond))
                                                     || !LevelChecked(Nastrond))
                                                 && HasEffectAny(Buffs.BattleLitany))
                                                || //Dives under Litany and Life of the Dragon
                                                (diveOptions is 3
                                                 && HasEffect(Buffs.LanceCharge))) //Dives under Lance Charge Feature
                                            {
                                                if (LevelChecked(DragonfireDive) && IsOffCooldown(DragonfireDive))
                                                    return DragonfireDive;
                                                if (LevelChecked(SpineshatterDive)
                                                    && GetRemainingCharges(SpineshatterDive) > 0)
                                                    return SpineshatterDive;
                                            }
                                        }
                        ...
                        if (comboTime > 0)
                        {
                            if (ChaosDoTDebuff is null
                                || ChaosDoTDebuff.RemainingTime < 6
                    ...
                    if (CanWeave(actionID))
                    {
                        ...
                        if (HasEffect(Buffs.PowerSurge))
                        {
                            ...
                            if (IsEnabled(CustomComboPreset.DRG_AoE_GeirskogulNastrond)
                                && LevelChecked(Geirskogul)
                                && ((gauge.IsLOTDActive && IsOffCooldown(Nastrond)) || IsOffCooldown(Geirskogul)))
                                return OriginalHook(Geirskogul);
                            ...
                            if (IsEnabled(CustomComboPreset.DRG_AoE_LifeSurge)
                                && !HasEffect(Buffs.LifeSurge)
                                && GetRemainingCharges(LifeSurge) > 0
                                && (HasEffect(Buffs.LanceCharge) || HasEffect(Buffs.RightEye))
                                && ((lastComboMove is CoerthanTorment && LevelChecked(CoerthanTorment))
                                    || (lastComboMove is SonicThrust
                                        && LevelChecked(SonicThrust)
                                        && !LevelChecked(CoerthanTorment))
                            ...
                            if (IsEnabled(CustomComboPreset.DRG_AoE_Dives)
                                && (IsNotEnabled(CustomComboPreset.DRG_AoE_Dives_Melee)
                                    || (IsEnabled(CustomComboPreset.DRG_AoE_Dives_Melee) && GetTargetDistance() <= 1)))
                            {
                                if (DiveOptions is 0 or 1 or 2 or 3
                                    && gauge.IsLOTDActive
                                    && LevelChecked(Stardiver)
                                    && IsOffCooldown(Stardiver)
                                    && CanWeave(actionID, 1.3)
                                    && IsOnCooldown(DragonfireDive))
                                    return Stardiver;
                                ...
                                if (DiveOptions is 0 or 1
                                    || //Dives on cooldown
                                    (DiveOptions is 2
                                     && ((LevelChecked(Nastrond) && gauge.IsLOTDActive) || !LevelChecked(Nastrond))
                                     && HasEffectAny(Buffs.BattleLitany))
                                    || //Dives under Litany and Life of the Dragon
                                    (DiveOptions is 3
                                     && HasEffect(Buffs.LanceCharge))) //Dives under Lance Charge Feature
