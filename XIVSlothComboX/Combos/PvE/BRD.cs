...
namespace XIVSlothComboX.Combos.PvE
{
    internal static class BRD
    {
        ...
        internal static bool SongIsWandererMinuet(Song value) => value == Song.WanderersMinuet;
        ...
        internal class BRD_AoE_oGCD : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                {
                    BRDGauge? gauge = GetJobGauge<BRDGauge>();
                    bool songWanderer = gauge.Song == Song.WanderersMinuet;
        ...
        internal class BRD_AoE_AdvMode : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Ladonsbite or QuickNock)
                {
                    ...
                    bool songWanderer = gauge.Song == Song.WanderersMinuet;
                    bool songMage = gauge.Song == Song.MagesBallad;
                    bool songArmy = gauge.Song == Song.ArmysPaeon;
        ...
        internal class BRD_OneButtonSongs : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is 放浪神的小步舞曲WanderersMinuet)
                {
                    ...
                    if (ActionReady(放浪神的小步舞曲WanderersMinuet) || (gauge.Song == Song.WanderersMinuet && songTimerInSeconds > 11))
                        return 放浪神的小步舞曲WanderersMinuet;

                    if (ActionReady(贤者的叙事谣MagesBallad) || (gauge.Song == Song.MagesBallad && songTimerInSeconds > 2))
                        return 贤者的叙事谣MagesBallad;

                    if (ActionReady(军神的赞美歌ArmysPaeon) || (gauge.Song == Song.ArmysPaeon && songTimerInSeconds > 2))
                        return 军神的赞美歌ArmysPaeon;
