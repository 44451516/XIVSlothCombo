...
namespace XIVSlothComboX.Combos.PvE
{
    internal static class WHM
    {
        ...
        internal static readonly Dictionary<uint, ushort> AeroList = new()
        {
            { Aero, Debuffs.Aero },
            { Aero2, Debuffs.Aero2 },
            { Dia, Debuffs.Dia }
        ...
        internal class WHM_SolaceMisery : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                return actionID is AfflatusSolace && BloodLilies == 3 ? AfflatusMisery : actionID;
            }
        }
        ...
        internal class WHM_RaptureMisery : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                return actionID is AfflatusRapture && BloodLilies == 3 ? AfflatusMisery : actionID;
            }
        }
        ...
        internal class WHM_CureSync : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                return actionID is Cure2 && !LevelChecked(Cure2) ? Cure : actionID;
            }
        }
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is All.即刻咏唱Swiftcast)
                {
                    ...
                    if (HasEffect(All.Buffs.Swiftcast))
                        return IsEnabled(CustomComboPreset.WHM_ThinAirRaise) && thinAirReady ? ThinAir : Raise;
                }
